	...
	<div class="reservemanagement" style="white-space: nowrap;height: 800px;"  >
		...
		<div style="padding: 0 0;">
			<el-row>
                ...
                <!-- <div style="font-size: 18px;font-weight: bold;padding-left: 20px;height: 40px;line-height: 40px;float: left;">Maintenance Order</div> -->
		...
		<div style="border-radius: 10px;background: white;height: 676px;width: 100%;" >
			<div>
				<el-table el-table :data="tableData" :header-cell-style="titlebg" style="min-width: 1108px;margin-bottom: 10px;height: 619px;border-radius: 10px;" row-key="id">
...
<!-- 					<el-table-column
          ...
          <el-table-column prop="salesNo" label="Sales No" sortable width="120px">
            <template slot-scope="scope">
              <span style="text-decoration: underline;cursor: pointer;" @click="to_factory(scope.row.salesNo)">{{scope.row.salesNo}}</span>
            </template>
          </el-table-column>

					<el-table-column label="Customer" >
						<template slot-scope="scope">
               <span style="text-decoration: underline;cursor: pointer;" @click="to_customer(scope.row.customer1.reference)" >{{scope.row.customer1==null?'':scope.row.customer1.customeName}}</span>
              <span>{{scope.row.customer2==null?'':'/'}}</span>
              <span style="text-decoration: underline;cursor: pointer;" @click="to_customer(scope.row.customer2.reference)" >{{scope.row.customer2==null?'':scope.row.customer2.customeName}}</span>
							<!-- <span style="text-decoration: underline;cursor: pointer;" @click="to_customer_detail(scope.row.customer1.reference)" >{{scope.row.customer1==null?'':scope.row.customer1.customeName}}</span> -->
              <!-- <span style="text-decoration: underline;cursor: pointer;" @click="to_customer_detail(scope.row.customer2.reference)">{{scope.row.customer2==null?'':' / '+scope.row.customer2.customeName}}</span> -->
            </template>
					</el-table-column>
...
<script>
	export default {
      ...
      to_factory(salesNo) {
        this.$router.push("/factoryManagement?type=salesOrder&salesNo=" + salesNo)
      },

      to_customer(uuid){
        this.$router.push("/CustomersManagement?type=to_customer&uuid="+uuid)
      },
