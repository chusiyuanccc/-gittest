	...
	<div class="reservemanagement">
		...
		<div style="border-radius: 10px;background: white;height: 676px;width: 100%;">
			<div>
				...
				<el-table :data="tableData.page.content" height="620" ref="singleTable01" :header-cell-style="titlebg" style='border-radius: 10px;'>
								...
								<el-menu style='background:rgb(245,108,108);margin-left: 10px;float: left;width: 80px;padding: 0;height: 32px;border-radius: 3px;'
								 ...
								 active-text-color="#fff">
									...
									<el-submenu index="1">
										...
										<el-menu-item v-if='scope.row.onLock==0'  index="2-1" @click="modify(scope.row.reference)" type="danger">Modify</el-menu-item>
										<el-menu-item v-if='scope.row.onLock==1'  index="2-1" style='color: #8C939D;'>Modify</el-menu-item>
...
// 			this.value6.push(new Date(2020, 11, 1))
		...
		methods: {
			...
			search() {
				...
				this.$api.post('/PlvRPaymentEndpoint/seekPage', {
			...
			confirm_detele() {
				...
				this.$api.get('/PlvRPaymentEndpoint/delete/' + this.statusid, null, r => {
					//console.log('****************************')
					this.search();
				});
			},
			...
			suoding_queren() {
				this.dialogVisibles = false
				this.$api.get('/PlvRPaymentEndpoint/onLock/' + this.statusid, null, r => {
					console.log(r);
					this.search()
				});
			},
			...
			usercreate() {
				this.$router.push("/NewDailyExpenditurepaymentManagement?type=new")
			},

			details(uuid) {
				this.$router.push("/NewDailyExpenditurepaymentManagement?type=detail&uuid=" + uuid)
			},
			modify(uuid) {
				this.$router.push("/NewDailyExpenditurepaymentManagement?type=alert&uuid=" + uuid)
			},
			topage(type) {
				if (type == 'user') {
