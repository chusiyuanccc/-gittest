<template>
	...
	<div class="reservemanagement">
		...
		<!-- 庫存列表 -->
		<!-- <div style="font-size: 18px;font-weight: bold;padding-left: 20px;">Invoice</div> -->
    <div style="font-size: 18px;font-weight: bold;padding-left: 20px;height: 40px;"></div>
		<div style="border-radius: 10px;background: white;height: 675px;width: 100%;">
          ...
          <el-table-column prop="supplier.name" label="Supplier" >
          </el-table-column>
					...
					<div class="action">
						<el-table-column fixed="right" label="Action"  align='center' style='background: red;' width='110'>
							<template slot-scope="scope">
      ...
      <div style="float: right;margin-right: 20px;" class="paginations">
        <el-select v-model="select_date" placeholder="select" style="float: left;width: 100px;font-size: 12px;height: 30px;margin-top: 10px;"
          @change="change_date()">
...
<script>
	export default {
      ...
      this.formInline.seek=this.$route.query.search_invoice==undefined?"":this.$route.query.search_invoice

      this.search_supplier=this.$route.query.search_supplier==undefined?"":this.$route.query.search_supplier
      ...
      indexMethod(index) {
        ...
        if(this.search_supplier!=""){
          ...
          this.$api.post('/invoice/count', {
            "invoiceNo": this.formInline.seek==""?null:this.formInline.seek,
            "supplier":{reference:this.search_supplier}
          }, r => {
            this.totalElements=r
          });

          this.$api.post('/invoice/'+this.value6[0].format("yyyy-MM-dd")+'/'+this.value6[1].format("yyyy-MM-dd")+'/'+(parseInt(this.page01)-1)+'/10', {
            "invoiceNo": this.formInline.seek==""?null:this.formInline.seek,
            "supplier":{reference:this.search_supplier}
          }, r => {
            this.tableData = r
...
// 				this.$api.post('/PlvRInvoiceEndpoint/seekPage', {
...
// 				});
			...
			confirm_detele() {
				...
				this.$api.get('/PlvRInvoiceEndpoint/delete/' + this.statusid, null, r => {
					//console.log('****************************')
					this.search();
				});
			},
			...
			usercreate() {
				this.$router.push("/NewDailyExpenditurereceiptManagement?type=new")
			},

			details(uuid) {
				this.$router.push("/NewDailyExpenditurereceiptManagement?type=detail&uuid=" + uuid)
			},
			modify(uuid) {
				this.$router.push("/NewDailyExpenditurereceiptManagement?type=alert&uuid=" + uuid)
			},
			topage(type) {
				if (type == 'grounp') {
