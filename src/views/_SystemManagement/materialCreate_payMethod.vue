<template>
  <div class="UserCreates">
    ...
    <div class="productSave"  style="max-width: 700px;margin: 0 auto;background: white;padding: 30px;border-radius: 10px;">
      <el-form :model="formLabelAlign"  label-width="120px" class="demo-ruleForm">
        <div style="width: 100%;color: white;background: deepskyblue;line-height: 30px;margin-bottom: 10px;">
          <strong style="margin-left: 10px;">Payment Method Setting</strong>
        </div>

        <el-form-item label="Method">
          <el-input placeholder="input" type="text" v-model="formLabelAlign.method" :disabled="type=='detail'"></el-input>
        </el-form-item>
    ...
    <!-- <div style="font-size: 14px;font-weight: bold;line-height: 30px;height: 40px;">
      ...
      <span class="fl" style="padding: 0 5px;">Payment Method Setting</span>
      <div style="width: 100px;border-bottom: 1px solid black;height: 14px;float: left;"></div>
    </div>
 -->
    <!-- ||||||||||||||||||||| -->
      ...
      <el-row style='line-height: 35px;'>
        <el-col :span="8">
          ...
          <p style="overflow: hidden;">
            <input v-model="formLabelAlign.method" :disabled="type=='detail'?true:false" class="tem_input">
          </p>
        </el-col>
    ...
    <!-- <div>
      <el-row style='line-height: 35px;'>
        <el-col :span="16">
          ...
          <p style="overflow: hidden;">
            <textarea rows="5" style="width: 100%;float: left;" v-model="formLabelAlign.remark"></textarea>
          </p>
        </el-col>
      </el-row>
    </div>
  ...
  export default {
    ...
    created: function() {
      ...
      if (this.type == 'detail') {
        ...
        // this.formLabelAlign.enable = this.$route.query.enable
        // this.formLabelAlign.id = parseInt(this.$route.query.id)
        // this.activateds = this.$route.query.enable == 1 ? true : false

        this.$api.get('/miscellaneous/paymentmethod/' + this.$route.query.uuid, null, r => {
        	this.formLabelAlign = r
    ...
    methods: {
      ...
      submitForm(formLabelAlign) {
        ...
        if (this.uuid != '') {
          ...
          this.loading = true
          this.$api.post('/miscellaneous/paymentmethod/save', this.formLabelAlign, r => {
          ...
          this.$api.put('/miscellaneous/paymentmethod/save', this.formLabelAlign, r => {
            // this.$router.push("/orderstatusManagement")
            history.back(-1)
          });
