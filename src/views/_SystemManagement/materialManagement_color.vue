					...
					<div class="grid-content bg-purple-dark" style="height: 40px;">
						...
						<div>
							<el-form :inline="true" :model="formInline" class="demo-form-inline">
								...
								<!-- <div style="font-size: 18px;font-weight: bold;padding-left: 20px;float: left;line-height: 40px;">Material</div> -->
					...
					<div class="action">
						<el-table-column fixed="right" label="Action"  align='center' style='background: red;white-space: nowrap;' width="200px">
							<template slot-scope="scope">
									...
									<el-button @click="alert(scope.row.reference,scope.row.name,scope.row.enable,scope.row.id)" type="danger" size="small">Modify</el-button>
                  <el-button @click="detele(scope.row.reference)" type="danger" size="small">Delete</el-button>
      ...
      indexMethod(index) {
        ...
        return (index+1) ;
			...
			confirm_detele() {
				...
				this.$api.delete('/miscellaneous/material/color/' + this.statusid+'/purge', null, r => {
					console.log('****************************')
					this.search();
				});
			...
			usercreate() {
				this.$router.push("/materialCreate_color?type=new")
			},

			detail(uuid) {
				this.$router.push("/materialCreate_color?type=detail&uuid=" + uuid)
			},
			alert(uuid,name,enable,id) {
				this.$router.push("/materialCreate_color?type=alert&uuid=" + uuid+"&name="+name+"&enable="+enable+"&id="+id)
			},
			//重置密码
			reset(name) {
