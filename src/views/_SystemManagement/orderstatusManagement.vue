		...
		<div style="border-radius: 10px;background: white;height: 675px;width: 100%;margin-top: 20px;" >
          ...
          <el-table-column prop="id" label="Id" width="80px" >
            ...
            >
					...
					<el-table-column prop="text" label="Status" width="200px">
					</el-table-column>
	...
	export default {
      ...
      indexMethod(index) {
        return ((parseInt(this.page01)-1)*10+index+1) ;
			...
			usercreate() {
				this.$router.push("/orderstatusCreate?type=new")
			},

			detail(uuid) {
				this.$router.push("/orderstatusCreate?type=detail&uuid=" + uuid)
			},
			alert(uuid) {
				this.$router.push("/orderstatusCreate?type=alert&uuid=" + uuid)
			},
			//重置密码
			reset(name) {
