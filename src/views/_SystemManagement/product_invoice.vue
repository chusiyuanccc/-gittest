<template>
	<div class="KaiZhiFapiao">
		<!-- <div style="font-size: 14px;font-weight: bold;line-height: 30px;height: 40px;">
			...
			<span class="fl" style="padding: 0 5px;">Product Invoice</span>
			<div style="width: 100px;border-bottom: 1px solid black;height: 14px;float: left;"></div>
		</div> -->
		<!-- ||||||||||||||||||||| -->
		<div>
       ...
       <div class="productSave"  style="max-width: 700px;margin: 0 auto;background: white;padding: 30px;border-radius: 10px;">
           <el-form :model="formLabelAlign"  label-width="120px" class="demo-ruleForm">
              ...
              <div style="width: 100%;color: white;background: deepskyblue;line-height: 30px;margin-bottom: 10px;">
                <strong style="margin-left: 10px;">Product Invoice</strong>
              </div>

              <el-form-item label="Item" prop="codePro">
                <el-input :disabled="type=='detail'" type="text" v-model="formLabelAlign.invoice.item"></el-input>
              </el-form-item>

              <el-form-item label="Quantity" prop="codePro">
                <el-input :disabled="type=='detail'" type="age" v-model="formLabelAlign.quantity"></el-input>
              </el-form-item>

              <el-form-item label="Amount" prop="codePro">
                <el-input :disabled="type=='detail'" type="text" v-model="formLabelAlign.invoice.amount"></el-input>
              </el-form-item>

              <el-form-item label="Currency" prop="codePro">
                <el-select v-model="formLabelAlign.invoice.currency" placeholder="select" style='width: 100%;' :disabled="type=='detail'?true:false">
                	<el-option v-for="item in unit" :label="item.label" :value="item.label">
                	</el-option>
                </el-select>
              </el-form-item>

              <el-form-item label="Invoice Date" prop="codePro">
                <el-date-picker v-model="formLabelAlign.invoice.invoiceDate" type="date" placeholder="select" style='width: 100%;'
                 :disabled="type=='detail'?true:false">
                </el-date-picker>
              </el-form-item>

              <el-form-item label="Invoice No" prop="codePro">
                <el-input placeholder="input" :disabled="type=='detail'" type="text" v-model="formLabelAlign.invoice.invoiceNo"></el-input>
              </el-form-item>

              <el-form-item label="Supplier" prop="codePro">
              ...
              <el-form-item label="Remark" prop="codePro">
                <el-input :disabled="type=='detail'" type="textarea" v-model="formLabelAlign.invoice.remark"></el-input>
              </el-form-item>

              <el-form-item label="Photo" prop="codePro">
                <div>
			...
			<!-- <el-row style='line-height: 35px;'>
				...
				<el-col :span="8">
					...
					<p style="overflow: hidden;">
						<input v-model="formLabelAlign.invoice.item" :disabled="type=='detail'?true:false" class="tem_input">
					</p>
				</el-col>
        ...
        <el-col :span="8">
        	...
        	<p style="overflow: hidden;">
        		<input v-model="formLabelAlign.quantity" :disabled="type=='detail'?true:false" class="tem_input">
        	</p>
        </el-col>
				...
				<el-col :span="8">
					...
					<p style="overflow: hidden;">
						<input v-model="formLabelAlign.invoice.amount" :disabled="type=='detail'?true:false" class="tem_input">
					</p>
				</el-col>
        ...
        <el-col :span="8">
        	...
        	<p style="overflow: hidden;">
				...
				<el-col :span="8">
					...
					<p style="overflow: hidden;">
						<input v-model="formLabelAlign.invoice.invoiceNo" :disabled="type=='detail'?true:false" class="tem_input">
					</p>
				</el-col>
...
<!--        <el-col :span="8">
        ...
        <el-col :span="8">
        	...
        	<p style="overflow: hidden;">
        		<textarea rows="3" style="width: 100%;" v-model="formLabelAlign.invoice.remark" ></textarea>
        	</p>
        </el-col> -->

                		<!-- <el-select style='width: 100%;' :disabled="type=='detail'?true:false" v-model="origin_input"
        :remote-method="querySearch_origin" @blur='changeSource' remote filterable placeholder="请选择">
...
<script>
	...
	export default {
      ...
      this.list_id=this.$route.query.invoice_list
      this.formLabelAlign.product.reference=this.list_id
        ...
        this.formLabelAlign.invoice.item=this.$route.query.invoice_item!=undefined?this.$route.query.invoice_item:''


			if (this.type == 'detail') {
				// this.$route.query.uuid
				this.$api.get('/product/'+this.list_id+'/invoice/' + this.$route.query.uuid, null, r => {
				...
				this.$api.get('/product/'+this.list_id+'/invoice/' + this.$route.query.uuid, null, r => {
					...
					this.imageUrl02=this.imageUrl02 = this.root + '/kernal/asset/' + r.invoice.image.reference
		...
		methods: {
			...
			uploadSectionFile02(param) {
				...
				this.$api.post('/kernal/asset/retain', bodyFormData, r => {
					...
					this.formLabelAlign.invoice.image.reference = r.reference
			...
			submitForm01(formLabelAlign) {
				...
				if (this.formLabelAlign.invoice.amount == '' || this.formLabelAlign.invoice.currency == '' ||
					this.formLabelAlign.invoice.invoiceNo == '' || this.formLabelAlign.invoice.invoiceDate == '' || this.formLabelAlign.invoice.item == '' ||
					...
					var that = this
					this.formLabelAlign.invoice.invoiceDate = new Date(this.formLabelAlign.invoice.invoiceDate).format("yyyy-MM-dd")
					this.formLabelAlign.invoice.amount=parseInt(this.formLabelAlign.invoice.amount)
					if (this.origin_input == '') {

						this.formLabelAlign.invoice.supplier = {
					...
					if (this.type == "alert") {
						...
						this.$api.post('/product/'+this.list_id+'/invoice/save', this.formLabelAlign, r => {
							//console.log('******************')
							// console.log(r)
							// this.$router.push("/DailyExpenditureinvoiceManagement")
              ...
              history.back(-1)
                    ...
                    history.back(-1)
							...
							this.$api.put('/product/'+this.list_id+'/invoice/save', this.formLabelAlign, r => {
								//console.log('******************')
								// console.log(r)
								// this.$router.push("/DailyExpenditureinvoiceManagement")
                history.back(-1)
