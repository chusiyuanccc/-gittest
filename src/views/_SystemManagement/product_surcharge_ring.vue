					...
					<div class="grid-content bg-purple-dark" style="height: 40px;">
						...
						<div>
							<el-form :inline="true" :model="formInline" class="demo-form-inline">
								...
								<!-- <div style="font-size: 18px;font-weight: bold;padding-left: 20px;float: left;line-height: 40px;">Rinig Surcharge</div> -->
        ...
        return (index+1) ;
			...
			search() {
				...
				// this.$api.get('/miscellaneous/orderstatus/'+(this.page01-1)+'/10',null, r => {
					this.$api.get('/product/surcharge/ring',null, r => {
			...
			confirm_detele() {
				...
				this.$api.delete('/product/surcharge/ring/' + this.statusid+'/purge', null, r => {
					console.log('****************************')
					this.search();
				});
			},
			...
			usercreate() {
				this.$router.push("/product_surcharge_ring_Create?type=new")
			},

			detail(uuid) {
				this.$router.push("/product_surcharge_ring_Create?type=detail&uuid=" + uuid)
			},
			alert(uuid) {
				this.$router.push("/product_surcharge_ring_Create?type=alert&uuid=" + uuid)
			},
			//重置密码
			reset(name) {
