					...
					<div class="grid-content bg-purple-dark" style="height: 40px;">
						...
						<div>
							<el-form :inline="true" :model="formInline" class="demo-form-inline">
								...
								<!-- <div style="font-size: 18px;font-weight: bold;padding-left: 20px;float: left;line-height: 40px;">Ring Size Surcharge Group</div> -->
						...
						<el-table-column fixed="right" label="Action"  align='center' style='background: red;' width="200px">
							<template slot-scope="scope">
								<div style="white-space: nowrap;">
									<el-button @click="handleClick(scope.row.reference,scope.row.name,scope.row.enable)" type="danger" size="small" >Modify</el-button>
...
<script>
	export default {
		...
		created: function() {
			...
			this.$api.get('/product/surcharge/ring/group/count', null, r => {
				console.log(r)
      ...
      indexMethod(index) {
        return ((parseInt(this.page01)-1)*10+index+1) ;
			...
			comfir(){
				...
				this.$api.delete('/product/surcharge/ring/group/'+this.dialogVisible02_reference+'/purge', null, r => {
						// this.branch_list = r.content,
						// 	this.branch = r.content[0].name
						this.page01=1
						this.search()
			...
			tocreate() {
				this.$router.push("/surcharge_group_create?type=new")
			},

			handleClick(id,name,enable) {

				// //console.log('-----' + id)
				this.$router.push("/surcharge_group_create?type=alter&id=" + id+'&name='+name+'&enable='+enable)
			...
			search() {
				...
				this.tableData02=[]
				this.$api.get('/product/surcharge/ring/group/'+(this.page01-1)+'/10', null, r => {
