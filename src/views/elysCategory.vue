            ...
            <div>
              <el-form :inline="true" :model="formInline" class="demo-form-inline">
                  <el-form-item style='float:left;'>
                      <h3>System Settings - Product Category</h3>
                  </el-form-item>
                ...
                <el-form-item style='float:right;'>
                  <el-button type="primary" @click="categorycreate" style='height:40px;padding: 0 10px;border:0;'>New</el-button>
                </el-form-item>
              </el-form>
            </div>
          </div>
...
<script>
  export default {
    ...
    created: function() {
      this.$api.get('/miscellaneous/category/count',null, r => {
    ...
    methods: {
      ...
      confirm_state(){
        this.$api.put('/miscellaneous/category/'+this.member_reference+'/group/'+this.options+'/assign',null, r => {
          this.search()
      ...
      search() {
        ...
        this.tableData=[]
        this.$api.get('/miscellaneous/category/'+(this.page01-1)+'/'+this.sum,null, r => {
            for (let i = 0; i < r.length; i++) {
                if (r[i].parent != null)
                {
                    r[i].name = this.getFullCategoryname(r[i]);
      ...
      categorycreate() {
        this.$router.push("/elysCategoryCreate?type=new")
      },
      return_guest() {
      ...
      detail(uuid) {
        this.$router.push("/elysCategoryCreate?type=detail&uuid=" + uuid)
      },
      alert(uuid) {
