  ...
  <div class="elysExpense">
            ...
            <div>
              <el-form :inline="true" :model="formInline" class="demo-form-inline">
                ...
                <el-form-item style='float:right;'>
                  <el-button type="primary" @click="expensecreate" style='height:40px;padding: 0 10px;border:0;'>New</el-button>
                </el-form-item>
              </el-form>
            </div>
          </div>
    ...
    <div style="border-radius: 10px;background: white;height: 676px;width: 100%;" >
      <div>
        <el-table :data="tableData" :header-cell-style="titlebg" style="width: 100%;text-align: center;margin: 0;padding: 0;height: 620px;border-radius: 10px;">
          ...
          <el-table-column prop="merchant" label="Merchant" align='center'>
          </el-table-column>
          <el-table-column prop="item" label="Item" align='center'>
          </el-table-column>
          <el-table-column prop="amount" label="Amount Spent" align='center'>
          </el-table-column>
          <el-table-column prop="formatted_date" label="Date" align='center'>
          </el-table-column>
    ...
    <div class="tishi">
      <el-dialog title="Tips" :visible.sync="dialogVisible" width="30%" :before-close="handleClose" top='20%'>
        <span>Do you really want to delete this Expense?</span>
...
<script>
  export default {
    ...
    methods: {
      ...
      confirm_state(){
        this.$api.put('/expense'+this.member_reference+'/group/'+this.options+'/assign',null, r => {
          this.search()
      ...
      search() {
        ...
        this.$api.get('/expense/'+(this.page01-1)+'/'+this.sum,null,r=> {
            for (let i = 0; i < r.length; i++) {
                r[i].formatted_date = new Date(r[i].date).toLocaleDateString().split("/").join("-");
            }
          this.tableData = r
      ...
      confirm_detele() {
        this.dialogVisible = false
        this.$api.delete('/expense/' + this.dialogVisible_reference + '/purge', null, r => {
          this.search();
        });
      },
      ...
      expensecreate() {
        this.$router.push("/elysExpenseCreate?type=new")
      },
      return_guest() {
      ...
      alert(uuid) {
        this.$router.push("/elysExpenseCreate?type=alert&uuid=" + uuid)
      },
