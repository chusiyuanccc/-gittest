...
<script>
  export default {
    ...
    methods: {
      ...
      confirm_state(){
        this.$api.put('/member/group'+this.member_reference+'/group/'+this.options+'/assign',null, r => {
          this.search()
      ...
      search() {
        ...
        this.tableData=[]
        this.$api.get('/member/group/0/100000',null, r => {
          this.tableData = r;
        });
      },
      onSubmit() {
      ...
      confirm_detele() {
        this.dialogVisible = false
        this.$api.delete('/member/group/' + this.dialogVisible_reference + '/purge', null, r => {
          this.search();
        });
      },
      detele(reference){
      ...
      orderstatuscreate() {
        this.$router.push("/elysGroupCreate?type=new")
      },
      return_guest() {
      ...
      detail(uuid) {
        this.$router.push("/elysGroupCreate?type=detail&uuid=" + uuid)
      },
      alert(uuid) {
        this.$router.push("/elysGroupCreate?type=alert&uuid=" + uuid)
      },
